-- Týdenní povinnosti jedné osoby se přednostně rozloží mezi její různé směny.
-- Navazuje na aplikovanou 03500; nemění základní kapacitní planner ani historii dokončení.
...
begin
  ...
  -- Před novým deterministickým rozložením odstraníme pouze odvozené přiřazení
  -- nedokončených týdenních povinností v dotčeném intervalu. Completion historie zůstává.
      ...
      select series.day_value::date as plan_date,
        ...
        coalesce((
          ...
          where used.active
            ...
            and used.due_from = duty.due_from
...
begin
  ...
  if function_source not like '%worker_responsibility_count%'
     or function_source not like '%order by generated_day.worker_responsibility_count%' then
    raise exception 'Planner nepreferuje rozdilne smeny pro tydenni povinnosti pracovnika.';
  end if;
