...
import { assignmentAppliesInMonth, assignmentOverlapsMonth, stableWorkerColor, workerInitials, workersForDate } from './workerPlanning.ts'
...
test('historické rozdělení respektuje dny, platnost a více pracovníků i pracovišť', () => {
  ...
  assert.deepEqual(workersForDate('2026-10-02', data).filter((item) => item.workerId === 'worker-dana').map((item) => item.areaLabel), ['1. patro', '3. patro'])
})
