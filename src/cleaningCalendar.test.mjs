...
test('Plán dne je read-only a měsíční interval načte po bezpečných blocích bez PostgREST ořezu', () => {
  ...
  const loader = readFileSync(new URL('./dynamicSchoolPlanLoader.ts', import.meta.url), 'utf8')
  const css = readFileSync(new URL('./styles.css', import.meta.url), 'utf8')
  const detail = app.slice(app.indexOf('function CalendarDayDetail'), app.indexOf('function CleaningCalendar'))
  ...
  assert.match(repository, /get_dynamic_school_cleaning_plan[\s\S]*target_from: chunkFrom, target_to: chunkTo/)
  assert.match(repository, /loadDynamicSchoolPlan\(\{/)
  assert.doesNotMatch(repository.slice(repository.indexOf('dynamicSchoolPlan:'), repository.indexOf('planOptions:')), /Promise\.all/)
  assert.match(loader, /rows\.length >= 1000/)
  assert.match(loader, /planReason:[\s\S]*assignedWorkerId:[\s\S]*plannerPriority:/)
