...
export default function App() {
  ...
  return <main className="app"><header><div><p className="eyebrow">ÚKLID ŠKOLY · ŠKOLA</p><h1>{section}</h1><p className="date">{todayLabel}</p></div><button className="avatar" aria-label="Odhlásit" title="Odhlásit" onClick={() => schoolRepository.signOut()}>{profile.full_name[0]}</button></header>{notice && <div className="notice">{notice}</div>}
    {section === 'Dnes' && <><section className="hero"><span>{isTestCleaningDay ? 'Testovací zobrazení pondělního úklidového dne.' : isCleaningDay ? 'Dnes je standardní úklidový den.' : 'Dnes není pravidelný úklidový den.'}</span><strong>{visible.filter(task => task.done).length} / {visible.length} hotovo</strong></section><h2>Nejdříve připravit cestu</h2><TaskList tasks={visible} onComplete={complete} /></>}
